...
def load_cut_to_rdd(input_file, result_file, cluster_num=CLUSTER_NUM, clu_iter=CLUSTERING_ITER,\
    ...
    largest_cluster = doc_vec
    clusters_list = [(1, doc_vec)] # initialize
    print 'repeated', now()
    for j in range(2, cluster_num+1):

        largest_num = clusters_list[0][0]
        largest_cluster = clusters_list[0][1]
        del clusters_list[0] # del cluster
        print 'cluster to divide', largest_num

        closest = largest_cluster.map(
                lambda (tid, feature):(closestPoint(feature, best_kPoints), (tid, feature))).cache()
        print 'total_count', closest.count()

        for i in range(K):
            single_cluster = closest.filter(lambda (index, (tid, feature)): index == i).values().cache()
            single_num = single_cluster.count()
            print 'single_num', i, single_num

            clusters_list.append((single_num, single_cluster)) # add cluster
        clusters_list.sort(key = lambda (x,y):x, reverse=True)

        largest_cluster = clusters_list[0][1] # the new round
        maximum_total_variance = 0
        best_kPoints = []
        for j in range(rb_iter):
            # clustering
            kPoints, tempDist, iter_count = clustering(largest_cluster, K, con_dist, clu_iter)
            # evaluation
            cluster_variance, total_variance = cluster_evaluation(largest_cluster, kPoints)
    ...
    for cluster in clusters_list:
        ...
        print >> f, per_size, total_similarity
