...
def external_evaluation(kPoints, global_center, g_length):
    obj_value = 0
    for i in range(len(kPoints)):
        tempDist = cosine_dist(kPoints[i][1], global_center, kPoints[i][2], g_length)
        obj_value += tempDist
    return obj_value
...
def load_cut_to_rdd(input_file, result_file, cluster_num=CLUSTER_NUM, clu_iter=CLUSTERING_ITER,\
    ...
    for i in range(ini_iter):
        ...
        # evaluation
        obj_value = external_evaluation(kPoints, global_center, g_length)

        # choose the best initial cluster
        if obj_value < minimum_value:
            minimum_value = obj_value
    ...
    for j in range(2, cluster_num+1):
        ...
        total_delta_variance = float("+inf")
        for key in updated_dict:
            if key < total_delta_variance:
                total_delta_variance = key

        print 'here', len(global_best_kPoints)
        for i in range(K):
            ...
            initial_distance = cosine_dist(global_best_kPoints[i][1], global_center, global_best_kPoints[i][2], g_length)
            ...
            improvement = minimum_value - initial_distance
            updated_dict[improvement] = single_cluster  # update dict
            updated_points_dict[improvement] = best_kPoints
            print 'improvement', improvement, minimum_value, initial_distance
