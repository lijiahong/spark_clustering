...
def closestPoint(p, centers):
    ...
    for i in range(len(centers)):
        tempDist = np.sum((p - centers[i][1]) ** 2)
        if tempDist < closest:
            closest = tempDist
            bestIndex = i
    return  bestIndex

def closestPointWithDist(p, centers):
    bestIndex = 0
    closest = float("+inf")
    for i in range(len(centers)):
        tempDist = np.sum((p - centers[i][1]) ** 2)
...
def load_cut_to_rdd(input_file, result_file):
    ...
    doc_vec = tfidf.groupByKey().mapValues(lambda feature : Vectors.sparse(num_term, feature).toArray())
    ...
    while tempDist > convergeDist:
        ...
        tempDist = sum(np.sum((kPoints[x][1] - y) ** 2) for (x, y) in newPoints)
    ...
    # evaluation
    closest = doc_vec.map(
            lambda (tid, feature):(closestPointWithDist(feature, kPoints), (tid, feature, 1)))
    ...
    global_distance = sum(np.sum((kPoints[x][1] - global_center[1]) ** 2) for x in range(len(kPoints)))
